...
Date.prototype.getNumberOfDays = function(thatday){ //now가 this / openday가 thatday
    ...
    // }
    let day = Math.trunc(interval/(1000*60*60*24)); // 소숫점에서 짤림
    day = Math.floor(interval/(1000*60*60*24)); // 소숫점에서 내림
    day = Math.round(interval/(1000*60*60*24)); // 소숫점에서 반올림
    day = Math.ceil(interval/(1000*60*60*24)); // 소숫점에서 올림
    return day;
};
// let now = new Date(); // 현재시점
