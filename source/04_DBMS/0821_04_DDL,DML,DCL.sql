...
-- DEPT유사한 DEPT01테이블 EMP와 유사한 EMP01테이블 생성
-- DEPT01 : DEPTNO(PK:숫자(2)), DNAME(문14), LOC(문자13byte)
CREATE TABLE DEPT01 (
    DEPTNO   NUMBER(2),
    DNAME    VARCHAR2(14),
    LOC      VARCHAR2(13),
...
DESC DEPT01;
INSERT INTO DEPT01 VALUES (10,  '재무','신림');  -- 트랜잭션에 쌓아 놓은 상태 

ROLLBACK; --트랜젝션에 쌓인 명령어 날리기

--EMP01; EMP(숫4; PK), ENAME(문10), JOB(문9), MGR(숫4), HIREDATE(날), SAL(숫7,2), COMM(숫7,2), DEPTNO(숫2;FK)
CREATE TABLE EMP01(
...
SELECT *FROM DEPT01;
COMMIT; -- 트랜잭션에 쌓여있는 명령어를 오라클 적용

DROP TABLE DEPT01; -- 참조하는 테이블이 있을 경우, 삭제 불가(삭제하려면 참조하는 테이블을 DROP한후 DROP추천)

-------------
--- ★ DML★ ---
-------------
-- 1. INSERT INTO 테이블명 VALUE(값1, 값2, ...값N);
        -- INSERT INTO 테이블명 (필드명1, 필드명2,...) VALUES (값1, 값2, ...);
SELECT * FROM DEPT01;
INSERT INTO DEPT01 VALUES (50, '법무', '서초');
INSERT INTO DEPT01 (LOC, DNAME, DEPTNO) VALUES('신림', '인사', 60);
INSERT INTO DEPT01 (DEPTNO, DNAME, LOC) VALUES(70, '인사', NULL); -- 명시적 NULL 입력
...
-- 연습문제 PDF PAGE1 (DDL, INSERT)
    --SAM01 테이블을 생성하시오 . 같은 이름의 테이블이 존재할 수 있으니 , DROP TABLE 로 삭제 후 생성하시오
/*
...
-- 2. UPDATE 테이블 명 SET [필드명1=값1, 필드명2=값2, ...필드명N=값N] [WHERE 조검]
DROP TABLE EMP01;

-- 서브 쿼리를 이용한 테이블 생성(제약 조건이 제외 된 데이터만 가져옴)
...
UPDATE EMP01 SET SAL = SAL+100 WHERE SAL <1200;
    ...
    -- EX. SMITH의 부서번호를 30, JOB을 'SALESMAN', SAL과 COMM을 500$씩 인상, 입사일을 오늘로, 상사는 BLAKE로  MGR
UPDATE EMP01 
    SET DEPTNO=30, 
    JOB='SALESMAN', 
    SAL=SAL+500, 
    ...
    HIREDATE= SYSDATE, --SYSDATE : 실행시점의 날짜와 시간
...
-- EX. SAM01에서 JOB이 정해지지 않은 사원을 삭제
SELECT * FROM SAM01;
...
--DML 연습문제 page3
--1. EMP 테이블과 같은 구조와 같은 내용의 테이블 EMP01 을 생성 (테이블이 있을시 제거한 후 )하 고 ,모든 사원의 부서번호를 30 번으로 수정합니다
DROP TABLE EMP01;
...
-----PRIMARY KEY와 UNIQUE 제약조건은 오늘쪽 나열, 밑에 나열 가능
-----NOT NULL 제약조건은 오른쪽에 나열 (밑에는 안됨)
CREATE TABLE DEPT1(
...
SELECT *FROM  DEPT1;
INSERT INTO DEPT1 VALUES (40, '법무부', 'SEOUL'); -- PK 제약조건 에러
...
INSERT INTO EMP1 (EMPNO, ENAME, DEPTNO) VALUES (1000, 'HONG', 10);

INSERT INTO EMP1(EMPNO, ENAME, SAL, DEPTNO)
    VALUES (1001, 'HONG',4,10); -- CHECK 제약조건 에러
