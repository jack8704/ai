...
-- 1. SELECT A문자 작성법(실행:CTRL+ENTER)
...
-- 2. 특정 열만 출력
DESC EMP; 
    -- ORACLE 명령어  EMP테이블 구조
SELECT ENAME, EMPNO,  SAL, HIREDATE FROM EMP; -- EMP테이블에서 EMPNO, ENAME, SAL열만 검색
SELECT ENAME AS "이 름", EMPNO AS "사 번",  SAL AS "급 여", HIREDATE AS "입사일" FROM EMP; 
SELECT ENAME "이 름", EMPNO "사 번",  SAL "급 여", HIREDATE "입사일" FROM EMP;
SELECT ENAME 이름, EMPNO "사 번",  SAL 급여, HIREDATE FROM EMP;
...
SELECT EMPNO NO, ENAME NAME, SAL FROM EMP WHERE SAL>3000;
    ...
    -- EX1. 사원이름(ENAME) 'A', 'B', 'C'로 시작하는 사원의 모든 필드   'A'<'AA'<'AAA'<'B'<'C'<'CA'<'CAA'...
...
-- 4. 특정 행만 출력 : WHERE절 (조건절)에서 논리연산자 :  OR, AND, NOT
        ...
        SELECT * FROM EMP 
             WHERE TO_DATE('82/01/01', 'RR/MM/DD')<=HIREDATE  AND HIREDATE <= TO_DATE('82/12/31', 'RR/MM/DD');
        --EX3. 10번 부서(DEPTNO) 이거나 JOB이 MANAGER인 직원 모든 필드
        SELECT * FROM EMP
            WHERE DEPTNO = 10 OR JOB='MANAGER';

-- 5. 산술연산자(SELECT절, WHERE절, ORDER BY절)
    -- EX. 연봉이 2400이상인 직원의 ENAME, SAL, 연봉(SAL*12)을 출력
    SELECT ENAME, SAL, SAL*12 "ANNUAL SAL"  --(3)번째
         FROM EMP                          --(1)번째
         WHERE SAL*12 >=2400         --(2) 번째    AS 사용 안됨
         ORDER BY "ANNUAL SAL"; -- 연봉순 오름차순(ASC) 정렬  // 내림차순(DESC)   (4)번째
    -- EX. 연봉이 10000이상인 직원의 ENAME, SAL, 연봉(SAL*12+COMM)을 출력
        --      산술연산의 결과는 null을 포함하면 결과도 null
        -- NVL(NULL일수도있는 필드명, 대체값) - 필드명과 대체값의 타입이 일치
        SELECT ENAME, SAL, SAL*12+NVL(COMM, 0) "연봉"
            FROM EMP      
            WHERE  SAL*12+NVL(COMM, 0) > 10000
            ORDER BY "연봉";
    -- EX. 모든 사원의 ENAME, MGR(상사사번)을 출력(MGR이 NULL 이면 'CEO'로 바꿔 출력)
        SELECT ENAME, NVL(TO_CHAR(MGR), 'CEO') MGR FROM EMP;
        DESC EMP;
 
 -- 6. 연결연산자(II) : 필드나 문자를 연결
 SELECT ENAME ||  '은' || JOB FROM EMP;
    --EX. 모든 사원에 대해서 "SMITH의 연봉은 XXXX$입니다." 출력하고 TITLE에는 MESSAGE.
...
----< 연습문제 > 
            ...
            SELECT ENAME || '은 ' || JOB || '업무이고 연봉은 ' || (SAL*12+NVL(COMM, 0))  || '다' MESSAGE 
                FROM EMP; 


-- 8. SQL 연산자(BETWEEN, IN, LIKE, IS NULL)
    -- (1) 필드명 BETWEEN A AND B : 필드명이 A부터 B까지 (A,B포함)
    --      필드명 NOT BETWEEN A AND B : 필드명이 A미만  B초과 (A,B 미포함)
         --EX. 급여가 1500이상이고 3000이하인 모든 필드 출력
            SELECT *  FROM EMP
                WHERE SAL BETWEEN 1500 AND 3000;  -- 반드시 A<B                                    --BETWEEN
        --EX. 82년도에 입사한 사람의 모든 필드 출력
            SELECT * FROM EMP
                WHERE HIREDATE BETWEEN TO_DATE('81/01/01', 'RR/MM/DD') AND TO_DATE('82/12/31', 'RR/MM/DD');
                
            SELECT * FROM EMP
                WHERE TO_CHAR(HIREDATE, 'RR/MM/DD') BETWEEN '81/01/01' AND '82/12/31';          
                
        --EX. 급여가 1500미만, 3000초과인 사람의 모든 필드 출력
            SELECT * FROM EMP WHERE SAL NOT BETWEEN 1500 AND 3000;                      --NOT BETWEEN
    
    -- (2) 필드명 IN (값1, 값2, ..............값N) : 필드명이 값1이거나 값2이거나,...값N인 경우
    --       필드명 NOT IN  (값1, 값2, ..............값N) : 필드명이 값1이 아니거나 값2이 아니거나,...값N이 아닌 경우
                --EX 부서코드가 10번이거나 30인 사람의 사번, 이름, 부서코드 출력
                SELECT EMPNO, ENAME, DEPTNO 
                    FROM EMP
                    WHERE DEPTNO IN (10, 30);
               --EX 부서코드가 10번,20,40번을 제외한 직원의 모든 필드 출력
               SELECT * 
                   FROM EMP
    ...
    --(3) 필드명 LIKE 패턴  : %(0글자이상), _(한글자)를 포함된 패턴
...
--(4) IS NULL (NULL인지를 검색할 때)
        ...
        -- EX. 상여금이 없는 사원의 모든 필드(COMM이 NULL이거나 0)
            SELECT * FROM EMP WHERE COMM IS NULL OR COMM = 0;
              -- SELECT * FROM EMP WHERE COMM IN (NULL, 0); -- 불가     COMM =NULL 불가
        -- EX. 상여금을 받는 사원의 모든필드 (COMM이 NULL이 아니고 COMM이 0이 아닌 사원)
            SELECT *FROM EMP WHERE COMM IS NOT NULL AND COMM <>0;
                
-- 9. 정렬(오름차순ASC, 내림차순DESC) : ORDER BY절
    SELECT ENAME, SAL, HIREDATE FROM EMP ORDER BY SAL, HIREDATE DESC;
                                                                                                -- 급여 오름차순, 급여 같으면 입사일 내림차순
...
*/
SELECT TO_CHAR(SAL, '9,999') 급여, TO_CHAR(HIREDATE, 'YYYY-MM-DD HH12:MI:SS') 입사일 FROM EMP;
...
--      업무, 급여를 검색하시오.
    ...
    FROM EMP
    WHERE TO_CHAR(HIREDATE,'RR/MM/DD') LIKE '81/%' AND JOB NOT IN 'SALESMAN';
