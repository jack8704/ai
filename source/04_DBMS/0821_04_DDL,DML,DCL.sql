...
/* SQL
 (1) DCL : 사용자 계정 생성(CREATE USER), 권한부여(GRANT), 권한박탈(REVOKE), 사용자 계정 삭제(DROP USER)
                 트렌젝션명령어(ROLLBACK, COMMIT)
 (2) DDL : 테이블 생성(CREATE TABLE), 테이블구조변경(ALTER TABLE). 테이블 삭제(DROP TABLE)
 (3) DML : CRUD
...
--- ★ DDL★ ---
-------------
-- 1. 태이블 생성(CREATE TABLE 테이블명...) : 테이블 구조 정의
        --ORACLE 타입 : NUMBER(38이하의 자릿수), DAT, VARCHAR2(4000이하의 바이트수)
CREATE TABLE BOOK(
    BOOKID         NUMBER(4),           --BOOKIN필드의 타입은 숫자4자리
    BOOKNAME  VARCHAR2(30),    --  BOOKNAME 필드의 타임은 문자30BYTE(한굴1자 =3BYTE)
