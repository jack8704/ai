...
-- ■■■■■■■■■■■
/*
my SQL 타입 : numeric(n, d), varchar(n), date(날짜만), datetime, timestamp(날짜시간)
정수 : tinyint(1byte), smallint(2byte), dediumint(3byte)
	  int/interger(4byte), bigint(8byte)
실수 : float(n, d ; 4byte), double(n, d;8byte)
문자 : char(n; 비추), text, longtext(4GB까지 저장가능)
*/
drop table if exists friend;
create table friend(
	no int			primary key auto_increment,  -- auto_increment 필드타입은 int로 sequence 처럼 자동적으로 증가
...
select database(); -- 현재들어와 있는 데이터베이스 
use devdb;
drop table if exists person; -- emp테이블 
...
-- 1. 사번, 이름, 급여를 출력
select pno, pname, sal from person;
...
-- 8. 사번, 이름, 상사이름(상사가 없는 사람도 출력하되 상사가 없는 경우 ★CEO★로 출력) – oracle과 다른 문법
	select w.pno, w.pname, ifnull(m.pname,'★CEO★') manager   -- nvl 대신 MySQL에서는 ifnull 함수 쓸수있음
		from person w left join person m
			on w.manager = m.pno;
    
-- 8-1 사번, 이름, 상사사번(상사가 없으면 ceo로 출력. ifnull함수의 매개변수의 타입이 상이해도 상관없음) – oracle과 다른 문법
	select pno, pname, ifnull(manager,'ceo') manager  -- nvl 대신 MySQL에서는 ifnull 함수 쓸수있음
		from person; 
	select pno, pname, if(manager is null, 'ceo', manager) manager from person;
-- 8-2 사번, 이름, 급여, 고액연봉자 or 일반연봉자
select pno, pname, sal, if(sal>=4000,'고액연봉자','일반연봉자') 연봉구분 from person;
...
select w.pno, w.pname, w.sal, d.dname, m.pname
	from person w , person m, division d
	where w.manager = m.pno
		and w.dno = d.dno;
        
select w.pno, w.pname, w.sal, d.dname, m.pname
	from division d 
		join person w on w.dno = d.dno
		left join person m on w.manager = m.pno;  -- 상사가 없는 사람도 출력
...
select now();  -- select절만으로도 실행
 ...
 -- select pname, to_char(hiredate, 'yy-mm-dd') 오라클 처럼 출력하고 싶다면
 -- date_format(날짜/시간필드, 포맷) => 문자형
		-- 포맷 : %Y(년도4자리), %y(년도2자리), %m(월2자리), %M(월이름), %d(일2자리), %H(24시간), %h(12시간), %p(오전,오후), %i(분), %s(초)
select pname, date_format(hiredate, '%Y년%m월%d일 %p %h:%i:%s') from person;    

-- select pname, to_char(sal, '9,999') from person; 오라클에서 사용 문법
 -- format(숫자필드, 소숫점자리수)
 select pname, format(sal, 0) sal from person;
 
 -- 오라클에서의 연결연산자 || 'smith는 manager다'  --MySQL에서의 ||는 OR연산자
 select concat (pname, '는', job,'다') message from person;
